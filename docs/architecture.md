...
The menu ships in every build, including the published one, but mounts only when `?qa=1` is requested; the flag is held in session storage and cleared with `?qa=0`. The boundary that matters is not
